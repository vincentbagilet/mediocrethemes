...
theme_mediocre <- function() {
  ...
  ggplot2::theme_minimal(base_size = 11, base_family = "opensans") %+replace%
    ggplot2::theme(
      ...
      plot.title = ggplot2::element_text(hjust = 0.05, size = ggplot2::rel(1.4), margin = ggplot2::margin(b = .5, unit = "cm")),
      plot.subtitle = ggplot2::element_text(hjust = 0.07, size = ggplot2::rel(1.2), face = "italic", vjust = 4.5),
      legend.position = "top",
