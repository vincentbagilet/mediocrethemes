...
This package provides both a ggplot theme and a color palette. Users can set the theme and color palette.
...
One can also easily det a background for all their plots by setting the parameter `background` of the functions `theme_mediocre` or `set_mediocre_all` to `TRUE`. The color of the background is predefined to  fit nicely the theme palette. One can of course still modify the background color by adding a layer to the ggplot theme: `theme(plot.background = element_rect(fill = "red"))`.
