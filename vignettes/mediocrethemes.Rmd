...
The easiest way to use the `mediocrethemes` package is through the `set_mediocre_all` function. It enables to set both the theme and the color scale for all graphs in a document. The user can call this function at the beginning of the document. Subsequent plots can be created using the standard ggplot commands, no additional commands are required. The user does not need to add `+ theme_mediocre()`, `+ theme_idid()` or `+ scale_mediocre_d()` to their plots to set the theme and the color, if they called `set_mediocre_all()` before. The following example illustrates this feature:
...
The function `set_mediocre_all` takes several arguments: `pal`, `theme`, `background`, `second_pair` and `gradient` along with the usual `base_size`, `base_family`, `base_line_size` and `base_rect_size`. The following subsections describe the use of the three first arguments.
...
set_mediocre_all(pal = "coty", theme = theme_idid)
...
plot_all_examples <- function(pal) {
  set_mediocre_all(pal = pal, theme = theme_idid)
  ...
  if (pal != "blackandwhite") {
    ...
    scale_mediocre_d(pal = pal, theme = theme_idid, gradient = "left") +
    labs(title = "A very nice title", subtitle = "A disappointing subtitle")
  ...
  plot7 <- ggplot(data = ggplot2::diamonds, aes(x = price, fill = color)) +
    geom_histogram(bins = 30) +
    scale_mediocre_d(pal = pal, theme = theme_idid, gradient = "right") +
    labs(title = "A very nice title", subtitle = "A disappointing subtitle") 
...
set_mediocre_all(pal = "idid", theme = theme_idid)
print("set_mediocre_all(pal = 'idid', theme = 'idid')")
...
set_mediocre_all(pal = "idid", theme = theme_idid, gradient = "left")
...
set_mediocre_all(pal = "idid", theme = theme_idid, gradient = "right")
...
```{r background}
set_mediocre_all(pal = "idid", theme = theme_idid, background = TRUE)
...
dat <- ggplot2::mpg |> 
  dplyr::mutate(year = ifelse(drv == "f", NA, year))

ggplot(data = dat, aes(x = drv)) +
    geom_bar(aes(fill = factor(year)), position = "dodge") +
    labs(title = "A very nice title", subtitle = "A disappointing subtitle")
```
...
```{r second_pair}
set_mediocre_all(pal = "idid", theme = theme_idid, second_pair = TRUE)
